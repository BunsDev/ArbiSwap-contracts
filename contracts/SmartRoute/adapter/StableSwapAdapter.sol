...
contract StableSwapAdapter is IRouterAdapter {
    ...
    function getAmountOut(
        ...
        if (baseSwap != address(0)) {
            try stableSwap.getTokenIndex(fromToken) returns (uint8 fromIndex) {
                ...
                i = ISwap(baseSwap).getTokenIndex(fromToken) + 1;
            }

            try stableSwap.getTokenIndex(toToken) returns (uint8 toIndex) {
                j = toIndex;
            } catch {
                j = ISwap(baseSwap).getTokenIndex(toToken) + 1;
            }
